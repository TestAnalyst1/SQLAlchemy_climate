...
def precipitation():
    ...
    lst_data_point = session.query(Measurement.date).order_by(Measurement.date.desc()).first()
    year_from_last_datapoint = dt.date(2017,8,23) - dt.timedelta(days= 365)

    year_prcp = session.query(Measurement.date, Measurement.prcp).\
    filter(Measurement.date >= year_from_last_datapoint, Measurement.prcp != None).\
...
def tobs():
    ...
    year_temp = session.query(Measurement.tobs).\
        filter(Measurement.date >= year_from_last_datapoint, Measurement.station == 'USC00519281').\
